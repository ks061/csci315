...
int main(int argc, char *argv[]) {
  ...
  if (fstat(fd, &file_info) == 0) {
    ...
    struct passwd **result_p = malloc(sizeof(struct passwd*));
    getpwuid_r(file_info.st_uid, pwd, buf, buflen, result_p);
    printf(" owner user name = %s\n", buf); // TO-DO: man getpwuid 
    struct group *grp = malloc(sizeof(struct group));
    struct group **result_g = malloc(sizeof(struct group*));
    getgrgid_r(file_info.st_gid, grp, buf, buflen, result_g); 
    printf(" owner group name = %s\n", buf); // TO-DO: man getgrgid 
