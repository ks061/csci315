...
dlist_add_front(struct dlist *l, void *ptr) {
	struct dnode *new_node_ptr = dnode_create();
	dnode_setdata(new_node_ptr, ptr);				

	if (l->front == NULL) {
		l->front = new_node_ptr;
		l->back = new_node_ptr;
	} else {
		new_node_ptr->next = l->front;
		l->front->prev = new_node_ptr;
		l->front = new_node_ptr;
	}
	l->counter++;
...
dlist_add_back(struct dlist *l, void *ptr) {
	...
	if (l->back == NULL) {
		...
		l->back->next = new_node_ptr;
		new_node_ptr->prev = l->back;
		l->back = new_node_ptr;
	}
	l->counter++;
...
dlist_remove_front(struct dlist *l) {
	...
	struct dnode *old_front = l->front;

	l->front = old_front->next;
	old_front->next = NULL;
	l->front->prev = NULL;

	ptr = dnode_destroy(old_front);
